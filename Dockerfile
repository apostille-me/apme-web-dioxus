...
COPY --from=build /work/target/release/apme-web-dioxus /usr/local/bin/apme-web-dioxus
USER app
...
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/apme-web-dioxus"]
