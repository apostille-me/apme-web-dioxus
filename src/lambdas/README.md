...
This nested crate proves that heavy and infrequent routes from `apostille-me/apme-web-dioxus` can be built and deployed independently of the main Dioxus web server. It does not change the existing server entrypoint, initialize Dioxus, open a listener, create a WebSocket, or construct the main application/database state.

## Entry points

| Binary | API Gateway route | Required body field | Intended workload |
|---|---|---|---|
| `heavy_document_preview` | `POST /api/heavy/document-preview` | `document_id` | Generate a document preview without booting Dioxus SSR. |
| `heavy_case_bundle` | `POST /api/heavy/case-bundle` | `case_id` | Build an immutable case bundle as an isolated job. |
